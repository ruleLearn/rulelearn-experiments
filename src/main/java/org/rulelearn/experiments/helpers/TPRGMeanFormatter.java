 ...
 */
public class TPRGMeanFormatter {
	...
	public static void main(String[] args) throws IOException { //pass e.g. "temp/input.csv" "temp/output.csv"
		if (args.length != 2) {
			System.out.println("Synopsis: TPRGMeanFormatter csvInputFilePath csvOutputFilePath");
			return;
		}
...
//        		cellsCount++;
        		...
        		for (String value : values) {
        			...
        			if (valuesCount < 2) { //a TPR
        				value = value.replace("1: ", "");
        				value = value.replace("0: ", "");
        				value = value.substring(0, value.indexOf(" "));
        			} else { //GMean
        				value = value.replace("Gmean: ", "");
        			}
        			...
        			if (valuesCount == 3) {
        				outputBuilder.append("$").append(savedValues[2]).append("^{").append(savedValues[0]).append("}").append("_{").append(savedValues[1]).append("}").append("$");
        			}
